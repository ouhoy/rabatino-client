...
async function fetchStudyCenterData() {
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch study center data')
    }
    ...
    formState.value = {
      ...
      hourlyRateRange: data.hourlyRateRange || [0, 0],
      has24Access: data.hasAccess, // Map hasAccess to has24Access
      rooms: data.rooms || []
    }
  } catch (error) {
    console.error('Error fetching study center:', error)
    fetchError.value = 'Failed to load study center data. Please try again.'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const studyCenterData = {
      ...
      hasAccess: formState.value.has24Access,
    ...
    if (!response.ok) {
      throw new Error('Failed to update study center')
    }

    const data = await response.json()
    console.log('Study center updated successfully:', data)
    router.push('/dashboard')
  } catch (error) {
    console.error('Error updating study center:', error)
  } finally {
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-12">
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        <button
            ...
            :disabled="isLoading"
        >
          {{ isLoading ? 'Updating Study Center...' : 'Update Study Center' }}
        </button>
      </div>
    </form>
  </div>
