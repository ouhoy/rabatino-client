...
async function fetchLibraryData() {
  try {
    ...
    formState.value = {
      ...
      hasDigitalAccess: data.hasDigitalAccess,
      operationHours: data.operationHours,
      hasPrinting: data.hasPrinting,
      hasStudyRooms: data.hasStudyRooms
    }
  } catch (error) {
    console.error('Error fetching library:', error)
    fetchError.value = 'Failed to load library data. Please try again.'
  } finally {
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-12">
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        <button
            ...
            :disabled="isLoading"
        >
          {{ isLoading ? 'Updating Library...' : 'Update Library' }}
        </button>
      </div>
    </form>
  </div>
