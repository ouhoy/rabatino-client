...
// Fetch business post data on mount
onMounted(async () => {
...
async function fetchBusinessData() {
  try {
    ...
    const response = await fetch(`${config.public.apiBaseUrl}/business/${slug}`)
    if (!response.ok) {
      throw new Error('Failed to fetch business post data')
    }
    ...
    fetchError.value = 'Failed to load business post data. Please try again.'
  } finally {
