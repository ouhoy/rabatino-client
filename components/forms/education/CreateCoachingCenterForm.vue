...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch(`${config.public.apiBaseUrl}/education/coaching-centers`, {
      method: 'POST',
      headers: {
    ...
    console.log('Coaching center created successfully:', data)
    // Reset form
    formState.value = {
      title: '',
      description: '',
  ...
  <form @submit.prevent="handleSubmit" class="space-y-12">
    ...
    <div class="mt-6 flex items-center justify-end gap-x-6">
      <button 
        ...
        :disabled="isLoading"
      >
        {{ isLoading ? 'Creating Center...' : 'Create Coaching Center' }}
      </button>
    </div>
  </form>
</template>
